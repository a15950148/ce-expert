...
**注入前必须验证（不可跳过，无论操作数形态如何）——至少确认以下四点：**

1. **目标动作触发时是否命中该指令**（开枪 / 受伤 / 拾取等）。
2. **玩家与敌人 / NPC 是否共用该指令**——分别触发目标行为与非目标行为，比对对象基址。
3. **寄存器上下文是否对应目标对象**（`rcx` / `rdi` 等基址寄存器这次命中指向谁）。
4. **重复触发时对象基址是否符合预期**（多次命中是否都是同一对象，或确实多对象）。

- 同一指令命中多个对象 → 用对象过滤、调用点 Hook 或其他上下文区分方法（见 `workflows/11-handle-shared-code.md`）。
- 已验证只命中目标对象 → 才可以**考虑**直接修改该位置。
...
> **MCP 工具权限由宿主 Agent 的 MCP 配置与权限系统管理。**
> 本 skill 不在 `allowed-tools` 中伪造、枚举或硬编码 MCP 权限；
> **运行时应通过实际 MCP 工具发现（tool discovery）确认可用能力**，而非假定。
...
即**禁止虚构 MCP 调用结果**，也禁止把 GUI 手动步骤写成"已自动完成"。
