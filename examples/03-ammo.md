...
> ⚠️ **先读本节末尾的「陷阱」再动手。** 下面这个流程只适用于**写入指令为弹药专用**的简单情形
> （典型是 `sub [addr],1` 这类立即数直写）。若你看到的是 `sub reg,reg` / 参数化调用，
> 说明命中的是全游戏共用的扣减函数，照下面做会导致**子弹无伤害**——直接跳到
> 「陷阱：命中的可能是全游戏共用的扣减函数」，那里是经两次失败后验证正确的做法。
